...
**Task:**
Provide a list item for each song of the format "[Song Title] [Artist] [YUP/NOPE]" with the word "YUP" in your response if the song fits the playlist and "NOPE" if it does not.
After each list item, provide a short explanation of why the song fits (or does not fit) the playlist followed by an empty line.
...
- **Lyrics:** ${lyrics}
- **Sentiment Score:** ${sentiment_score} (A measure of the song's overall sentiment; positive values indicate positive sentiment, negative values indicate negative sentiment.)
...
- **Key:** ${key} (The key of the track; 0-11, where 0 is C, 1 is C♯/D♭, 2 is D, and so on.)
- **Liveness:** ${liveness} (Detects audience presence; values above 0.8 suggest a live performance.)
- **Loudness:** ${loudness} dB (Overall track loudness in decibels; typical values range between -60 and 0 dB.)
